...
const BuyingReceipt = () => {
  const { setDisplayFighterBuyConfirmation } = useContext(BlockchainContext);

  const handleProceedButton = (e) => {
    e.preventDefault();
    setDisplayFighterBuyConfirmation(false);
      ...
      <div className="flex h-screen justify-center items-center">
        <div className="flex flex-col justify-center items-center bg-black p-2 md:p-4 border-4 border-red-900 rounded-xl">
          <p className="text-white md:text-xl text-2xl pb-2 md:pb-4 text-center text-gradient">
            Your desired Fighter has been bought! Check them out in your
            Barracks!
          </p>
          <button
            type="button"
